...
def stateLLStep(lastState, thisState, predictors, g):
	#If thisState == 0 (susceptible), then the rate of transition
	#out is going to be dictated by b or 1-b. If thisState == 1 (infectious)
...
all_series_prob, all_updates = theano.map(fn = stateSeriesProb, sequences = [initPredictors, statePredictors, stateMatrix, g])
...
if __name__ == '__main__':
	...
	#A vector of village level logits for state autocorrelation
	g = np.array([0.01, 0.1, 0.05])
